...
class Market:
    ...
    def run_step(self):
        ...
        inv_bias = np.clip(abs(self.drl_agent.inventory) / 4.0, 1.0, 3.5)
        # inv_bias = np.clip(abs(self.drl_agent.inventory) / 5.0, 1.0, 3.0) # CHANGED SLIGHTLY
        reward += 0.002 * (1 - min(abs(inv) / 8.0, 1.0)) # ADDED
