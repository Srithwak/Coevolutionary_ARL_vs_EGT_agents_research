...
# - NEW METRICS: Alpha, Beta, Sortino, Win Rate, Profit Factor
...
class Market:
    ...
    def __init__(self, nn_architecture, drl_params, ohlcv_filename):
        ...
        self.history = {
            ...
            "epsilon": [],
            "reward": [],
            "trade_pnl": [], # NEW: Track pure trading PnL per step
            "egt_prop_aggressive": [],
            "egt_prop_passive": [],
            "egt_prop_random": [],
            "egt_prop_momentum": [],
    ...
    def run_step(self):
        ...
        self.history["trade_pnl"].append(trade_profit_total) # Log trade PnL
    ...
    def print_summary(self):
        ...
        - NEW: Alpha, Beta, Sortino, Win Rate, Profit Factor
        - SAVES to 'performance_metrics.txt' in results folder
        """
        ...
        mid_prices = np.array(self.history["mid_price"], dtype=float)
        ...
        if len(mid_prices) > 1:
            market_returns = np.diff(mid_prices) / mid_prices[:-1]
        else:
            market_returns = np.array([0.0])
        ...
        # Beta: Cov(Agent, Market) / Var(Market)
        if len(step_returns) > 1 and len(market_returns) > 1:
            covariance = np.cov(step_returns, market_returns)[0][1]
            market_variance = np.var(market_returns)
            beta = covariance / (market_variance + 1e-8)
        else:
            beta = 0.0

        # Alpha (Annualized): R_p - (Beta * R_m)
        # Assuming Risk Free Rate = 0 for simplicity
        alpha = (mean_ret - (beta * np.mean(market_returns))) * steps_per_year

        # Sortino Ratio: Mean / Downside Deviation
        target_return = 0.0
        downside_returns = step_returns[step_returns < target_return]
        downside_std = np.std(downside_returns) if len(downside_returns) > 0 else 1e-8
        sortino = (mean_ret * np.sqrt(steps_per_year)) / (downside_std + 1e-8)

        # Win Rate
        wins = np.sum(step_returns > 0)
        total_trades_steps = len(step_returns)
        win_rate = (wins / total_trades_steps) * 100.0

        # Profit Factor: Gross Profit / Gross Loss
        gross_profit = np.sum(step_returns[step_returns > 0])
        gross_loss = np.abs(np.sum(step_returns[step_returns < 0]))
        ...
        lines.append("")
        lines.append(f"Sharpe (per-step signal/noise): {sharpe:.3f}")
        lines.append(f"Sharpe (annualized): {sharpe_annualized:.3f}")
        lines.append(f"Sortino (annualized): {sortino:.3f}")
        lines.append("")
        lines.append(f"Max Drawdown: {max_drawdown * 100:.2f}%")
        lines.append("")
        lines.append("--- Advanced Metrics ---")
        lines.append(f"Alpha (Annualized): {alpha:.4f}")
        lines.append(f"Beta (vs Market): {beta:.4f}")
        lines.append(f"Win Rate: {win_rate:.2f}%")
        lines.append(f"Profit Factor: {profit_factor:.2f}")
        lines.append("")
        lines.append("--- Step Return Distribution ---")
        lines.append(f"Mean Return per Step: {mean_ret:.6f}")
        lines.append(f"Std Dev of Return per Step: {std_ret:.6f}")
        lines.append(f"Skewness of Returns: {skew_ret:.6f}")
        lines.append(f"Kurtosis of Returns: {kurt_ret:.6f}")
        lines.append("")
        lines.append("Final EGT Population Distribution:")
        for i, strat in enumerate(self.egt_strategies):
            lines.append(f"  {strat.capitalize()}: {self.egt_proportions[i]:.3f}")
        lines.append("=" * 30)
        ...
        # --- Save to File ---
        # Reconstruct save_dir logic (should match plot_results)
        ...
        filepath = os.path.join(save_dir, "performance_metrics.txt")
        try:
